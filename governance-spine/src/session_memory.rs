...
//!   Tier 2 — StrategicMemory: Cross-session actor profiling
//!
...
/// goes from theoretical to load-bearing. Uses a bounded recent window with oldest records dropped first.
...
impl SessionMemory {
    ...
    /// Export fingerprint for cross-session strategic memory
...
// ═══════════════════════════════════════════════════════════════════════════
//  TIER 2 — STRATEGIC MEMORY (Cross-Session)
// ═══════════════════════════════════════════════════════════════════════════
...
/// Strategic-memory advice when a new session starts
#[derive(Debug, Clone)]
...
impl StrategicMemory {
    ...
    /// Called when a session ends — adds the fingerprint to strategic memory.
        ...
        match self.actors.get(actor_id) {
            ...
            Some(profile) => {
                let advisory = if profile.campaign_detected {
                    Some(format!(
                        "STRATEGIC MEMORY ADVISORY: Actor {} — {} sessions, {} escalated. \
