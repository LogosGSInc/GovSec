...
use governance_spine::EvidenceGovernedPipeline;
...
fn start_server(max_connections: usize, socket_timeout: Duration) -> std::net::SocketAddr {
    ...
    let pipeline = Arc::new(EvidenceGovernedPipeline::default_for_test().expect("pipeline init"));
...
fn normal_health_request_succeeds() {
    ...
    assert!(
        ...
        "expected ok:true body, got: {resp}"
    );
}

#[test]
fn authenticated_evidence_endpoint_is_available() {
    let addr = start_server(64, Duration::from_secs(10));
    let mut stream = TcpStream::connect(addr).expect("connect");
    let request = format!(
    ...
    assert!(
        resp.contains("\"epoch_id\":\"EVP-"),
        "expected evidence epoch, got: {resp}"
    );
}
...
fn oversized_header_line_rejected_431() {
    ...
    stream
        .write_all(format!("X-Huge: {huge_value}\r\n").as_bytes())
        .unwrap();
    stream.write_all(b"\r\n").unwrap();
    let resp = read_all_available(&mut stream, Duration::from_secs(5));
    assert!(
...
fn silent_client_is_dropped_by_socket_timeout_not_held_forever() {
    ...
    if let Ok(n) = result {
        assert_eq!(
            n, 0,
            "expected EOF (server closed idle connection), got {n} bytes"
        );
    }
    assert!(
